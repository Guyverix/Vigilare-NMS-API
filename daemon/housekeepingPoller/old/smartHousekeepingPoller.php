...
while (true) {
  ...
  */
  ...
  if ($housekeepingCount > 0) {
   ...
   // If we can update a heartbeat in the db, the database is working! (duh)
   $utcDate=gmdate("Y-m-d H:i:s");
   $db->query("INSERT INTO heartbeat VALUES(\"mysql\", \"iteration_60\",now(), \"12345\") ON DUPLICATE KEY UPDATE  lastTime=\"$utcDate\" , pid=\"12345\" ");
   $db->execute();
  ...
  if ( ! empty($trapd)) {
   ...
   foreach($activeEvents as $validEvent) {
     if ( $validEvent['eventName'] == "snmptrapd") {
