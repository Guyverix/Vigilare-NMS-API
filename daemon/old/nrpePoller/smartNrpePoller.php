...
if (!@flock($pidFile, LOCK_EX | LOCK_NB)) {
  ...
  if ( $daemonState == "stop" ) {
    ...
    $dbKill = new Database();
    $dbKill->query("DELETE FROM heartbeat WHERE device=\"smartNrpePoller\" AND component=\"iteration_$iterationCycle\" ");
    $dbKill->execute();
...
while (true) {
  ...
  if ( ! empty($db->error) || ! isset($db)) {
    while ( ! empty($db->error) || ! isset($db)) {
      sendAlarm("Daemon has lost its database connection", 5, "smartNrpePoller-database-" . $iterationCycle);
      unset($db);
      $logger->error("Database failure $this->error");
      sleep(20);
      $db = new Database();
    }
    sendAlarm("Daemon has restored its database connection", 0, "smartNrpePoller-database-" . $iterationCycle);
    $logger->info("Database reconnected");
  }

  // Update heartbeat each iteration
  $utcDate=gmdate("Y-m-d H:i:s");
  $db->query("INSERT INTO heartbeat VALUES(\"smartNrpePoller\", \"iteration_$iterationCycle\",now(), \"$pid\") ON DUPLICATE KEY UPDATE  lastTime=\"$utcDate\" , pid=\"$pid\" ");
  $db->execute();
  if ( ! empty($db->error)) {
  ...
  */
  unset($nrpeChecks);
  $db->query("SELECT * FROM monitoringPoller WHERE iteration=$iterationCycle AND type='nrpe' AND (hostname !='' OR hostGroup !='')");
  $db->execute();
  $nrpeCheckCount=$db->rowCount();
  $nrpeChecks=$db->resultset();
  $logger->info("smartNrpePoller table query for monitors to poll returned $nrpeCheckCount rows");
  $nrpeChecks=json_decode(json_encode($nrpeChecks),true);
  ...
  foreach ($nrpeChecks as $keyIndex => $valueCheck) {
    ...
    if ( ! empty($valueCheck['hostGroup'])) {
      ...
      $db->execute();
      $nrpeHosts=$db->resultset();
      $nrpeHosts=json_decode(json_encode($nrpeHosts),true);
      // print_r($nrpeHosts);
      // echo $nrpeHosts[0]['hostname'] . "\n" ;
      // Add or append values into hostname
      if (empty($nrpeChecks["$keyIndex"]['hostname'])) {
        $nrpeChecks["$keyIndex"]['hostname']= $nrpeHosts[0]['hostname'];
      }
      else {
        $nrpeChecks["$keyIndex"]['hostname'] .= ', ' . $nrpeHosts[0]['hostname'];
      }
      $cleanHostnameList=explode(',', $nrpeChecks["$keyIndex"]['hostname']) ;
      $cleanHostnameList=array_map('trim', $cleanHostnameList);
      $cleanHostnameList=array_unique($cleanHostnameList);
      $nrpeChecks["$keyIndex"]['hostname'] = implode(',' , $cleanHostnameList);
    }
  }
  // Create nrpeChecks[?][hostlist] with all values specific to host
  foreach ($nrpeChecks as $key => $appendDetails) {
    // echo "KEY " . $key . " VALUE ";
    ...
    foreach($hostListings as $FQDN) {
      ...
      foreach ($hostDetail as $hostDetails) {
        $nrpeChecks["$key"]['hostlist'][$hostDetails['hostname']][$hostDetails['component']][$hostDetails['name']] = $hostDetails['value'];
      }
    $nrpeChecks["$key"]['hostlist'][$hostDetails['hostname']]['address'] = $hostDetails['address'];
    }
  }
...
/*
    [1] => Array
            [id] => 38
            [checkName] => checkNetworkConnections-443
            [checkAction] => checkNetworkConnections -a 40 50 443 SSLConns
            [type] => nrpe
            [iteration] => 300
            [storage] => graphite
            [hostname] => nas01.iwillfearnoevil.com,myth-front01.iwillfearnoevil.com,guyver-office.iwillfearnoevil.com,guyver-myth.iwillfearnoevil.com
            [hostGroup] => nrpeGeneric, linuxGeneric
            [hostlist] => Array
                    [nas01.iwillfearnoevil.com] => Array
                            [SNMP] => Array
                                    [snmpSetup] => public, v2c, 161
                            [address] => 192.168.15.125
                    [myth-front01.iwillfearnoevil.com] => Array
                            [SNMP] => Array
                                    [snmpSetup] => public, v2c, 161
                            [address] => 192.168.15.80
                    [guyver-office.iwillfearnoevil.com] => Array
                            [Environment] => Array
                                    [Production] => preprod
                            [Location] => Array
                                    [Coordinates] => 48.0840N, 121.9687W
                            [SNMP] => Array
                                    [snmpSetup] => public, v2c, 161
                            [SSH] => Array
                                    [sshPassword] => 123Password_OUCH
                                    [sshUser] => someUser_OUCH
                            [address] => 192.168.15.58
                    [guyver-myth.iwillfearnoevil.com] => Array
                            [SNMP] => Array
                                    [snmpSetup] => public, v2c, 161
                            [address] => 192.168.15.74
*/
  ...
  foreach($nrpeChecks as $nonObjnrpeChecks) {
    ...
    foreach ($nonObjnrpeChecks['hostlist'] as $hostKey => $hostValue ) {
      // print_r($hostnames);
      $hostname = $hostKey;
      ...
      $logger->info("Running nrpe command: " . $nrpePath . " -H " . $hostname . " -c " . $checkCommand);
      $nrpeHostLoopCount++ ;
      $result=exec( "$nrpePath -H $hostname -c $checkCommand 2>&1" , $output, $result_code);
      // echo "complete\n";
      if ($result_code == 0 ) {
        // Success
        ...
        //echo "NRPE alarm: ". $result . " severity " . " check_name " . $checkName . " checkHost " . $hostname . " completeOutput " . $completeOutput . "\n";
        $logger->error("NRPE alarm: ". $result . " severity " . " check_name " . $checkName . " checkHost " . $hostname . " completeOutput " . $completeOutput);
        // Database seems to choke on pipes
        $alarmCompleteOutput=preg_replace('/\|/',' ', $completeOutput);
        if (empty($result)) {
          $result="NRPE command failure. Args Used: " . $checkCommand;
        }
...
//        }
      ...
      foreach ($cleanMetrics as $checkMetric) {
        ...
        if (is_numeric($metricValue)) {
          ...
          $graphiteKey=$hostnameGraphite . ".nrpePoller." . $checkName . '.' . $mName . '';
      ...
      $db->execute();
      $nrpeChecks=$db->resultset();
      $logger->info("nrpePoller stored result output to performace database for " . $hostname . " " . $checkName . " completeOutput " . $completeOutput);
    }
    unset ($checkCommand);  // after running the eval
    unset ($completeOutput); // looks like a merge happening.  Testing to strip
  ...
  if ( ($sleepDate + $iterationCycle) <= $timeNow ) {
    ...
    sendAlarm("Daemon poller had an iteration overrun.  Confirm daemon is not overloaded in logs delta is " . $timeDelta . " seconds", 2, "smartNrpePoller-iterationComplete-" . $iterationCycle);
    $logger->error("Iteration overrun.  Cycle took $timeDelta seconds beyond the iteration defined for $nrpeCheckCount different checks with $nrpeHostLoopCount hosts");
    $newIterationCycle=1;
  }
  else {
    $timeDelta=($timeNow - $sleepDate);
    foreach($activeEvents as $validEvent) {
      if ( $validEvent['eventName'] == "smartNrpePoller-iterationComplete-" . $iterationCycle ) {
        sendAlarm("Daemon poller iteration complete.  Daemon is not overloaded in logs delta is " . $timeDelta . " seconds", 0, "smartNrpePoller-iterationComplete-" . $iterationCycle);
      }
    }
    $logger->info("Iteration complete.  Cycle took $timeDelta seconds to complete against $nrpeCheckCount different checks with $nrpeHostLoopCount hosts");
...
function signalHandler($signal) {
  ...
  $dbShutdown->query("DELETE FROM heartbeat WHERE device=\"smartNrpePoller\" AND component=\"iteration_$iterationCycle\" ");
  $dbShutdown->execute(); 
  global $pidFile;
  global $pid;
  ftruncate($pidFile, 0);
