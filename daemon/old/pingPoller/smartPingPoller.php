...
$pingPath='/usr/lib/nagios/plugins/';
...
if (!@flock($pidFile, LOCK_EX | LOCK_NB)) {
  ...
  if ( $daemonState == "stop" ) {
    ...
    $dbKill = new Database();
    $dbKill->query("DELETE FROM heartbeat WHERE device=\"pingPoller\" AND component=\"iteration_$iterationCycle\" ");
    $dbKill->execute();
...
while (true) {
  ...
  if ( ! empty($db->error) || ! isset($db)) {
    while ( ! empty($db->error) || ! isset($db)) {
      sendAlarm("Daemon has lost its database connection", 5, "pingPoller-database-" . $iterationCycle);
      unset($db);
      $logger->error("Database failure $this->error");
      sleep(20);
      $db = new Database();
    }
    sendAlarm("Daemon has restored its database connection", 0, "pingPoller-database-" . $iterationCycle);
    $logger->info("Database reconnected");
  }
  // Update heartbeat each iteration
  $utcDate=gmdate("Y-m-d H:i:s");
  $db->query("INSERT INTO heartbeat VALUES(\"smartPingPoller\", \"iteration_$iterationCycle\",now(), \"$pid\") ON DUPLICATE KEY UPDATE  lastTime=\"$utcDate\" , pid=\"$pid\" ");
  $db->execute();
  ...
  */
  unset ($pingChecks);
  $db->query("SELECT * FROM monitoringPoller WHERE iteration=$iterationCycle AND type='ping' AND (hostname !='' OR hostGroup !='')");
  $db->execute();
  $pingCheckCount=$db->rowCount();
  $pingChecks=$db->resultset();
  $pingChecks=json_decode(json_encode($pingChecks),true);
  $logger->info("pingPoller table query for monitors to poll returned $pingCheckCount rows");
  $pingHostLoopCount=0;

  // Figure out if we have hostGroups and append the hosts matching with the hosts that may be defined in hostname
  foreach ($pingChecks as $keyIndex => $valueCheck) {
    ...
    if ( ! empty($valueCheck['hostGroup'])) {
      ...
      $db->execute();
      $pingHosts=$db->resultset();
      $pingHosts=json_decode(json_encode($pingHosts),true);
      // print_r($pingHosts);
      // echo $pingHosts[0]['hostname'] . "\n" ;
      // Add or append values into hostname
      if (empty($pingChecks["$keyIndex"]['hostname'])) {
        $pingChecks["$keyIndex"]['hostname']= $pingHosts[0]['hostname'];
      }
      else {
        $pingChecks["$keyIndex"]['hostname'] .= ', ' . $pingHosts[0]['hostname'];
      }
      $cleanHostnameList=explode(',', $pingChecks["$keyIndex"]['hostname']) ;
      $cleanHostnameList=array_map('trim', $cleanHostnameList);
      $cleanHostnameList=array_unique($cleanHostnameList);
      $pingChecks["$keyIndex"]['hostname'] = implode(',' , $cleanHostnameList);
    }
  }
  // Create pingChecks[?][hostlist] with all values specific to host
  foreach ($pingChecks as $key => $appendDetails) {
    // echo "KEY " . $key . " VALUE ";
    ...
    foreach($hostListings as $FQDN) {
      ...
      foreach ($hostDetail as $hostDetails) {
        $pingChecks["$key"]['hostlist'][$hostDetails['hostname']][$hostDetails['component']][$hostDetails['name']] = $hostDetails['value'];
      }
    $pingChecks["$key"]['hostlist'][$hostDetails['hostname']]['address'] = $hostDetails['address'];
    }
  }
  ...
  foreach($pingChecks as $nonObjPingChecks) {
    // Retrieve each check
    foreach( $nonObjPingChecks as $k => $v ) {
      // echo "KEY " . $k . " VALUE " . $v . "\n";
      if ($k == 'checkName') {
        $checkName="$v";
      }
    ...
    // print_r($nonObjpingChecks);

    foreach ($nonObjPingChecks['hostlist'] as $hostKey => $hostValue ) {
      // print_r($hostnames);
      $hostname = $hostKey;
      ...
      $concatName=$pingPath . $checkName;
      // echo "Running ping command: " . $concatName . " values " . $checkCommand . "\n";

      $logger->info("Running ping command: " . $concatName . " " . $checkCommand);
      $pingHostLoopCount++ ;
      ...
      if ($result_code == 0 ) {
        ...
        //echo "Ping alarm: ". $result . " severity " . " check_name " . $checkName . " checkHost " . $hostname . " completeOutput " . $completeOutput . "\n";
        $logger->error("Ping alarm: ". $result . " severity " . " check_name " . $checkName . " checkHost " . $hostname . " completeOutput " . $completeOutput);
        // Database seems to choke on pipes
        $alarmCompleteOutput=preg_replace('/\|/',' ', $completeOutput);
        if (empty($result)) {
          $result="Ping command failure. Args Used: " . $checkCommand;
        }
      ...
      */
      ...
      foreach ($seporateMetric as $dirtyMetric) {
        if (strpos($dirtyMetric, '=') !== false) {
          ...
          $logger->debug("ping " . $checkName . " metric data appears to be legitamite " . $dirtyMetric);
      ...
      foreach ($cleanMetrics as $checkMetric) {
        ...
        if (is_numeric($metricValue)) {
          ...
          $graphiteKey=$hostnameGraphite . ".pingPoller." . $checkName . '.' . $mName . '';
      ...
      $db->execute();
      $pingChecks=$db->resultset();
      $logger->debug("pingPoller stored result output to performace database for " . $hostname . " " . $checkName);
    }
  ...
  if ( ($sleepDate + $iterationCycle) <= $timeNow ) {
    ...
    sendAlarm("Daemon poller had an iteration overrun.  Confirm daemon is not overloaded in logs delta is " . $timeDelta . " seconds", 4, "smartPingPoller-iterationComplete-" . $iterationCycle);
    $logger->error("Iteration overrun.  Cycle took $timeDelta seconds beyond the iteration defined for $pingCheckCount different checks with $pingHostLoopCount hosts");
    $logger->warning("Setting next iteration to start in one second");
    $newIterationCycle=1;
  }
  else {
    $timeDelta=($timeNow - $sleepDate);
    foreach($activeEvents as $validEvent) {
      if ( $validEvent['eventName'] == "smartPingPoller-iterationComplete-" . $iterationCycle ) {
        sendAlarm("Daemon poller iteration complete.  Daemon is not overloaded in logs delta is " . $timeDelta . " seconds", 0, "smartPingPoller-iterationComplete-" . $iterationCycle);
      }
    }
    $logger->info("Iteration complete.  Cycle took $timeDelta seconds to complete against $pingCheckCount different checks with $pingHostLoopCount hosts");
...
function signalHandler($signal) {
  ...
  $logger2->warning("Daemon shutdown");
  $dbShutdown= new Database();
  $dbShutdown->query("DELETE FROM heartbeat WHERE device=\"smartPingPoller\" AND component=\"iteration_$iterationCycle\" ");
  $dbShutdown->execute();
  global $pidFile;
  global $pid;
  ftruncate($pidFile, 0);
