...
*/
  ...
  foreach ($metric as $singleMetric) {
    ...
    $renderOptions = array( "--start", $start, "--end", $end, "-w 800", "--alt-autoscale-max", "--rigid", "--vertical-label=rpm", "--title=\"" . $singleMetric['title'] . "\"",
                            "DEF:" . $singleMetric['metricName'] . "=" . $file . ":" . $singleMetric['metricName'] . ":AVERAGE",
                            'AREA:' . $singleMetric['metricName'] . colorList(0) . ' LINE1:' . $singleMetric['metricName']  . colorList(0) . ':"RPM " GPRINT:' . $singleMetric['metricName'] . ':LAST:%6.2lf%s\\\n', );
