...
*/
  ...
  // lower-limit is useful when there is not a lot of fluctuation in the values. making the graph look odd
  $renderOptions = array( "--start", $start, "--end", $end, "-w 800", "--lower-limit=0", "--alt-autoscale-max", "--rigid", "--vertical-label=temp", "--title=\"$title\"",
                 "DEF:fan=".$file.":".$metricName.":AVERAGE",
                 'AREA:fan' .colorList(0) . ' LINE1:fan' . colorList(0) . ':"Temp " GPRINT:fan:LAST:%6.2lf%s\\\n',
               );
