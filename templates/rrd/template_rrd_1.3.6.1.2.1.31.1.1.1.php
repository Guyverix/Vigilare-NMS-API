...
*/
    ...
    foreach ($dataToBeInserted as $k => $v) {
      ...
      switch ($k) {
        ...
        case "iso.3.6.1.2.1.31.1.1.1.16.$interface":
         if ( $v == "false" ) { $v = 1; } else { $v = 0; }
         $clean[$interface]['dataToBeInserted'][] = array( 'name' => 'PromiscuousMode', 'value' => $v, 'type' => 'GAUGE') ;
         break;
        case "iso.3.6.1.2.1.31.1.1.1.17.$interface":
         if ( $v == "false" ) { $v = 1; } else { $v = 0; }
         $clean[$interface]['dataToBeInserted'][] = array( 'name' => 'ConnectorPresent', 'value' => $v, 'type' => 'GAUGE') ;
         break;
      }
    }
  ...
  foreach ($mapped as $interfaces) {
    ...
    // if ($hostname='dell-6248-home.iwillfearnoevil.com') { echo "RAW DS " . $rrdReturnDataCreate . "\n"; } // DEBUG
