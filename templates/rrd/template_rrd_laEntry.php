  ...
  */
  ...
  foreach ($dataToBeInserted as $k => $v) {
    if ( strpos($k, '3.6.1.4.1.2021.10.1.1.') !==false) {
      $SOMENAMEIndexNumber=preg_replace('/.*.3.6.1.4.1.2021.10.1.1./','', $k);
  ...
  */
  ...
  foreach ($list as $SOMENAME) {
    ...
    foreach ($dataToBeInserted as $k => $v) {
      ...
      switch ($k) {
        case "iso.3.6.1.4.1.2021.10.1.2.$SOMENAME":
          ...
          $v=preg_replace('/\ /', '_', $v);   // Convert space to _
          $v=preg_replace('/-/', '_', $v);    // Convert - to _
          $clean[$SOMENAME]['laEntry'] = "$v";
          break;
        case "iso.3.6.1.4.1.2021.10.1.1.$SOMENAME":
          $clean[$SOMENAME]['dataToBeInserted'][] = array( 'name' => "laIndex", 'value' => "$v", 'type' => 'UNUSED');  // So we know what the table is returning, but we are not saving the data as a metric
          break;
        case "iso.3.6.1.4.1.2021.10.1.3.$SOMENAME":
          $v=preg_replace('/.*.\./', '' , $v);
          $clean[$SOMENAME]['dataToBeInserted'][] = array( 'name' => "laValue", 'value' => "$v", 'type' => 'GAUGE');
          break;
        case "iso.3.6.1.4.1.2021.10.1.5.$SOMENAME":
          $v =  preg_replace('/\D/', '',$v); // Strip everything but numbers
          $v=ltrim(rtrim($v));               // strip whitespace
          $clean[$SOMENAME]['dataToBeInserted'][] = array( 'name' => "laValueInt", 'value' => "$v", 'type' => 'GAUGE');
          break;
        case "iso.3.6.1.4.1.2021.10.1.4.$SOMENAME":
          //$v =  preg_replace('/\D/', '',$v); // Strip everything but numbers
          $v=ltrim(rtrim($v));               // strip whitespace
          $clean[$SOMENAME]['dataToBeInserted'][] = array( 'name' => "laAlarm", 'value' => "$v", 'type' => 'GAUGE');
          break;
        case "iso.3.6.1.4.1.2021.10.1.100.$SOMENAME":
          if ( $v == 'noError' ) { $v = 0; } else { $v = 1; }
          $v =  preg_replace('/\D/', '',$v); // Strip everything but numbers
          $v=ltrim(rtrim($v));               // strip whitespace
          $clean[$SOMENAME]['dataToBeInserted'][] = array( 'name' => "laActiveAlarm", 'value' => "$v", 'type' => 'GAUGE');
          break;
      }
    }
  ...
  foreach ($mapped as $someNames) {
    ...
    $rrdRootFile=$hostname . "/snmp/cpu/load/" . $someNames['laEntry'] . "_32.rrd";  // really int values, but JIC I find some good 64 counters dont want to recode

    $rrdReturnData[$someNames['laEntry']]['fileName'] = $rrdRootFile;
    ...
    $rrdReturnData[$someNames['laEntry']]['create']=$rrdReturnDataCreate;
    $rrdReturnData[$someNames['laEntry']]['update']=$rrdReturnDataUpdate;
  } // end foreach
